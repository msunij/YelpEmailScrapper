...
domains_not_to_crawl = ('https://www.facebook.com',
...
def crawl(keywords):
	...
	query = {'q': keywords}
	url = 'http://www.google.com/search?' + urllib.urlencode(query) + '&start=' + str(0)
	data = retrieve_html(url)
	# 	print("data: \n%s" % data)
	for url in google_url_regex.findall(data):
		if not url.startswith(domains_not_to_crawl):
			print("url:", url)
			continue
	# for url in google_adurl_regex.findall(data)[:3]:
	# 	if not url.startswith(domains_not_to_crawl):
	# 		print "url:", url
	...
	email_set = find_emails_2_level_deep(url)
	if (len(email_set) > 0):
		return email_set
	else:
		return False
	...
	for link in links:
		...
		try:
			...
			if link.startswith("#") or link == "/":
				...
				link_filtered.append(url.scheme+'://'+url.netloc+link)
			elif link.startswith(url.scheme+'://'+url.netloc+link):
